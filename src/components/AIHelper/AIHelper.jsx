  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://babelforgeserver.vercel.app/ai/ai/prompt",
        {
          method: "POST",
      ...
      <div className="drop-shadow-xl bg-white p-6 rounded-lg border border-[#e5e7eb] h-[634px] w-full">
        <div className="flex flex-col space-y-1.5 pb-6">
          ...
          <p className="text-sm w-full text-[#6b7280] leading-3">
            Your Personal Guide. AI Assistant can make mistakes. Check important
            info.
          </p>
        </div>
          ...
          {chatHistory.map((chat, index) => (
            <div
              ...
              className={`flex gap-3 my-4 text-sm ${
                chat.sender === "AI Assistant" ? "justify-start" : "justify-end"
              }`}
            >
              <span className="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
                ...
                >
                  <Image
                    src={chat.sender === "AI Assistant" ? logo : userbw}
                    alt={chat.sender}
                    className="w-full"
                  />
                </div>
