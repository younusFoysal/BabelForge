  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {tasks?.map((task) => (
        <div
          key={task._id}
          className="p-4 bg-white border-2 border-blue-600 rounded-xl shadow-2xl text-center cursor-pointer w-full dark:bg-gray-800"
        >
          <h1 className="text-2xl font-bold uppercase text-blue-600">
            {task.tname}
          </h1>
          <h2 className="text-lg text-gray-700">Status: {task.tproces}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4">
            ...
            >
              <span className="mx-auto">Update Status</span>
            </button>
            <button
