...
const Reviews = () => {
  ...
  const axiosCommon = useAxiosCommon();
    ...
    useEffect(() => {
        setValue("name", session?.user?.name || 'Your Name');
    }, [session, setValue]);
                    ...
                    <div className="flex flex-col  w-full py-4 ">
                        ...
                        <div className="flex flex-col py-6 space-y-5 ">
                            ...
                            <div className="flex items-center lg:space-y-0 space-y-2 lg:flex-nowrap flex-wrap space-x-4 ">
                                ...
                                {/* emoji 2 */}
                                <button onClick={() => handleRatingClick(2)}
                                    type="button"
                                    className='hover:shadow-xl duration-200' >
                                    <Image
                                        className='rounded-md px-4 py-2 border-[1px] border-gray-300 hover:border-blue-400 hover:border-2 object-cover '
                                ...
                                {/* emoji 3 */}
                                <button onClick={() => handleRatingClick(3)}
                                    type="button"
                                    className='hover:shadow-xl duration-200' >
                                ...
                                {/* emoji 4 */}
                                <button onClick={() => handleRatingClick(4)}
                                    type="button"
                                    className='hover:shadow-xl duration-200'
                                ...
                                {/* emoji 5 */}
                                <button onClick={() => handleRatingClick(5)}
                                    type="button"
                                    className='hover:shadow-xl duration-200'
                                    title="Rate 1 stars" aria-label="Rate 1 stars">
                                    <Image
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="flex flex-col w-full mt-4  ">
                                ...
                                <input  {...register("name")} name="name"
                                    type="text" defaultValue={session?.user?.name || 'Your Name'} disabled class="block w-full p-2.5 rounded-md text-sm border-gray-300 border-[1px] mb-5" />


                                <p className='pb-2 opacity-90'>Whats your overall experience?</p>
                                <textarea required  {...register("message")}
                                    rows="5" placeholder="please share your thoughts..." className="p-2 rounded-md resize-none text-black border-gray-300 border-[1px]"></textarea>
