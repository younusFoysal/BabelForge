    ...
    return (
        <div className="mt-6 overflow-x-auto rounded-xl bg-white px-6 shadow lg:px-4">
            <table className="min-w-full border-collapse border-spacing-y-2 border-spacing-x-2">
                ...
                <tbody className="bg-white border-gray-300 divide-y divide-gray-200 sm:divide-none sm:space-y-4">
                    {transactions?.map((item, id) => (
                        <tr key={item._id} className="sm:space-y-2 sm:py-4">
                            <td className="whitespace-no-wrap py-4 text-left text-sm text-gray-600 sm:px-3 sm:py-2">
                                {new Date(item?.date).toLocaleString()}
                            </td>
                            <td className="whitespace-no-wrap py-4 text-sm font-normal text-gray-600 sm:px-3 sm:py-2">
